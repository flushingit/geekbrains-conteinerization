Не совсем понял про количество реплик при обновлении. 
Вроде MaxSurge 0 должно решать проблему, ограничивая количество
запущенных реплик, если я правильно понял.

И про рестарты пода, при попытке изменить политику рестартов - он ругается

![Failure](./images/on_failure.png)

Но в целом всё заработало

![Kubedoom](./images/kubedoom.png)
